...
check(float(gp2.context_token_dropout) == 0.15, "B2  per-token dropout = 0.15")
...
check(float(gp2.syn_gate_bias_init) == -5.0, "B4  gate bias = -5.0")
...
check(all(float(g.syn_gate_bias) == -5.0 for g in agg.groups), "all 5 groups carry the gate")

...
if [ "${FLUX2_VARIANT}" != "4b" ]; then
  echo "Goal-pose prior Stage1 currently supports FLUX2_VARIANT=4b only, got ${FLUX2_VARIANT}" >&2
  exit 1
fi

MODEL_CONFIG="configs/model/imagewam_flux2_klein_4b_goal_prior_stage1.yaml"
TASK_NAME="libero_flux2_klein_4b_goal_prior_stage1"
...
imagewam_print_config TASK_NAME FLUX2_VARIANT DATA_ROOT FLUX2_SRC FLUX2_MODEL_PATH FLUX2_AE_MODEL_PATH QWEN_CACHE_DIR QWEN_CONTEXT_LEN ACTION_INIT TRAIN_NORM_STATS WANDB_MODE
